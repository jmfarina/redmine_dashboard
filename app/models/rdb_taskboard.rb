  ...
  def build
    ...
    when :tracker
      trackers.each do |tracker|
        self.add_group RdbGroup.new("tracker-#{tracker.id}", tracker.name, RdbGroup::TRACKER, tracker.id, :accept => Proc.new {|issue| issue.tracker == tracker })
      end
    when :priority
      IssuePriority.find(:all).reverse.each do |p|
        self.add_group RdbGroup.new("priority-#{p.position}", p.name, RdbGroup::PRIORITY, p.id, :accept => Proc.new {|issue| issue.priority_id == p.id })
      end
    when :assignee
      self.add_group RdbGroup.new(:assigne_me, :rdb_filter_assignee_me, RdbGroup::ASSIGNEE, User.current.id, :accept => Proc.new {|issue| issue.assigned_to_id == User.current.id })
      self.add_group RdbGroup.new(:assigne_none, :rdb_filter_assignee_none, RdbGroup::ASSIGNEE, nil, :accept => Proc.new {|issue| issue.assigned_to_id.nil? })
      assignees.sort_by(&:name).each do |principal|
        next if principal.id == User.current.id
        self.add_group RdbGroup.new("assigne_#{principal.id}", principal.name, RdbGroup::ASSIGNEE, principal.id, :accept => Proc.new {|issue| !issue.assigned_to_id.nil? and issue.assigned_to_id == principal.id })
      end
    when :category
      issue_categories.each do |category|
        self.add_group RdbGroup.new("category-#{category.id}", category.name, RdbGroup::ISSUE_CATEGORY, category.id, :accept => Proc.new {|issue| issue.category_id == category.id })
      end
      self.add_group RdbGroup.new(:category_none, :rdb_unassigned, RdbGroup::ISSUE_CATEGORY, nil, :accept => Proc.new {|issue| issue.category.nil? })
    when :version
      versions.each do |version|
        self.add_group RdbGroup.new("version-#{version.id}", version.name, RdbGroup::VERSION, version.id, {:accept => Proc.new {|issue| issue.fixed_version_id == version.id}})
      end
      self.add_group RdbGroup.new(:version_none, :rdb_unassigned, RdbGroup::VERSION, nil, :accept => Proc.new {|issue| issue.fixed_version.nil? })
    when :project
      projects.each do |project|
        self.add_group RdbGroup.new("project-#{project.id}", project.name, RdbGroup::PROJECT, project.id, :accept => Proc.new {|issue| issue.project_id == project.id })
      end
    when :parent
      issues.where(id: issues.pluck(:parent_id).uniq).uniq.all.each do |issue|
        self.add_group RdbGroup.new("issue-#{issue.id}", issue.subject, RdbGroup::PARENT_ISSUE, issue.id, :accept => Proc.new { |sub_issue| sub_issue.parent_id == issue.id })
      end
      self.add_group RdbGroup.new("issue-others", :rdb_no_parent, RdbGroup::PARENT_ISSUE, nil, :accept => Proc.new { |issue| issue.parent.nil? and issue.children.empty? })
    end
